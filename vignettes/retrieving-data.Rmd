...
```{r get_vt, echo=TRUE, eval=FALSE}
aedes_responses <- get_vt(aedes_ids[1:5])
aedes_responses[[1]]
```
```{r get_vt_secret, include=FALSE}
aedes_responses <- get_vt(aedes_ids[1:5], check_src = FALSE, connections = 5)
aedes_responses[[1]]
```
...
df <- get_vt(ids[1:5], check_src = FALSE, connections = 5) %>%
  extract_vt(
    cols = c("Interactor1Genus", "Interactor1Species", "Latitude", "Longitude"),
    returnunique = TRUE
  )
...
df <- search_vt("Bemisia")[1:6] %>%
  get_vt(check_src = FALSE, connections = 6) %>%
  extract_vt(cols = c("DatasetID", "Interactor1Genus", "Interactor1Species", "Interactor2Genus", "Interactor2Species"))
```
...
```{r searcherror_fixed_secret, include=FALSE}
df <- search_vt_smart("Interactor1Genus", "contains", "Bemisia")[1:6] %>%
  get_vt(check_src = FALSE, connections = 6) %>%
  extract_vt(cols = c("DatasetID", "Interactor1Genus", "Interactor1Species", "Interactor2Genus", "Interactor2Species"))
unique(df$Interactor1Genus)
