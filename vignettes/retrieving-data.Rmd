...
```{exemplar, echo=TRUE, message=FALSE, results=FALSE}
...
If you are using Linux, sometimes `httr2` fails to communicate with the VectorByte servers. As such on these systems it is better to start with a basereq with reduced server authentication.
<!-- Shim to make sure vignette builds on ubuntu-->
```{r basereq2, echo=TRUE, message=FALSE, results=FALSE}
basereq <- vb_basereq(unsafe=TRUE)
```
...
```{r get_vt_ids}
basereq %>% get_current_vt_ids() %>% head(n=20)
```
...
```{r search_vt}
aedes_ids <- basereq %>% search_vt("Aedes aegypti")
aedes_ids
```
...
aedes_responses <- basereq %>% get_vt_byid(aedes_ids[1:5])
aedes_responses[[1]]
```
```
## <httr2_response>
...
basereq %>% get_extract_vt_byid_chunked(
  aedes_ids[1:5],
  chunksize = 2,
  cols = c("LocationText", "OriginalTraitName"),
  returnunique = TRUE)
...
ids <- basereq %>% search_vt("Aedes aegypti")
df <- basereq %>%
  get_vt_byid(ids) %>%
  extract_vt_data(
    cols=c("Interactor1Genus", "Interactor1Species", "Latitude", "Longitude"),
