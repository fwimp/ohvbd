...
```
## Deconstructing `get_ad()`

That call above to `get_ad()` used some slightly odd parameters, so let's break that down a bit.
...
The metric argument tells `get_ad()` what type of data it should retrieve from AREAdata.

There are many valid options for this including temperature and humidity. (See [AREAdata](https://pearselab.github.io/areadata/download-links/) or run `?get_ad()` in the R console for a full list).

### `gid`

The gid argument tells `get_ad()` the spatial scale to download (with higher numbers corresponding to finer scales, at the cost of larger download sizes).
...
Typically, the cache location is a folder within your current working directory called `.adcache`, however if you want to overwrite it (such as if you want to share the cache between different scripts) you can provide an alternative path to save data to, as we have done here.
...
get_ad(metric = "relhumid", gid = 1, use_cache = TRUE, cache_location = cachepath)
```
